    ...
    try {
      ...
      if (!findUser || findUser.length < 0) {
        ...
        if (CryptrConverter.decrypt(findUser.password) === password) {
          ...
          // get data profile
          let getProfiles = await userProfile.findOne({
...
exports.saveScore = async function (req, res) {
  ...
  try {
    let findDataTotalScore = await gameHistoryTotal.findOne({
      userId: userId
    })
    ...
    if (!findDataTotalScore) {
      ...
      let updateTotalGameScore = await gameHistoryTotal.findOneAndUpdate({
        userId: userId
      }, findDataTotalScore)
...
exports.getScore = async function (req, res) {
  ...
  try {
    ...
    res.send({
      statusCode: 200,
      message: 'Successful to load game score !',
      result: getScore
    })
  } catch (error) {
    console.log(error);
    res.status(500).send({
      statusCode: 500,
      message: "failed to load game data score!",
    });
  }
}
